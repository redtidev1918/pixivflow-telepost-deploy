...
* **ci:** make release step idempotent when release-please already created the release ([dbe8507](https://github.com/redtidev1918/pixivflow-telepost-deploy/commit/dbe8507bc776aba5ee7edb5411ac271a50ba05b1))
...
- **生产拓扑收敛为「两个应用 + 一个时钟」**：PixivFlow 与 TelePost 现在是各自独立的应用、
  独立机器、独立持久卷，不再混部、不再共用内存与进程生命周期。PixivFlow 平时停止以省钱，
  由定时触发器经平台代理唤醒，跑完自己的账本后自行退出；TelePost 常驻不休眠，冷启动对
  用户不再可见（此前「机器人不回复」即由此而来）。
- **用户投稿全链路恢复，且仍然人工审核**：私聊投稿、媒体收集、可选字段、预览编辑、取消、
  审核、批准发布全部可用；PixivFlow 的每日成果经投稿接口进入同一审核队列，必须人工批准后
  才发布到频道，没有任何自动批准或直发路径。
- **重复实现清理**：删除了部署仓库里与两端重复的调度账本、审核与 Telegram webhook 实现，
  以及基于它们的旧上线门禁与生产验收脚本；新增守护测试，防止「同一件事的第二份权威实现」
  再次出现（历史事故正是两个都想当权威的一方吞掉了用户投稿）。
- **可验证性**：新增一组只读运维脚本（生产校验、投稿端/执行端冒烟、webhook 归属、镜像与
  提交号核对），一条命令即可得出生产是否符合契约的结论，且任何情况下都不打印密钥。
...
- 修正 `docker/combined.Dockerfile` 默认 `TELEPOST_IMAGE` ARG 为 2.10.28：
  v1.8.22 的预构建组合镜像（`latest` / `1.8.22`）默认嵌入了 TelePost 2.10.27，
  与发布说明不一致；本版确保拉取预构建镜像的用户也运行 2.10.28。
...
- PixivFlow 升级到 **2.10.26**：调度守护进程自我修复——启动补跑错过的 cron、
  计划级卡死看门狗（默认 30 分钟）、启动时 pixivflow.db `quick_check`
  （损坏自动隔离重建并通知审核群）；另清理运行时死代码。
- TelePost 升级到 **2.10.27**：审计驱动死代码清理（零行为变更）。
