...
| cron-job.org 控制台配置（PRIMARY 表达式） | **已配置**（操作者于 2026-09-13 在控制台手工创建）：`pixivflow-primary-bot1-daily`（next run 22:00 Asia/Shanghai）、`pixivflow-primary-bot2-daily`（next run 22:10 Asia/Shanghai）。这是 SaaS 侧事实，仓库无法直接读取；2026-09-13 09:50Z 的 readiness 核验只确认仓库 canonical contract（`control-plane/src/cron-map.ts` 的 `primaryCron` 与 [调度运维手册](../operations/scheduling.md) 的 URL / headers / 凭据边界）与控制台配置**应当一致**，真实 fire 仍需今晚的触发历史作证。 |
...
> **2026-09-13 14:00Z/14:02Z 窗口的真实结果取到之后，必须回到本节补写时间戳与处置结果（cron-job.org
> 触发历史 + 执行端 admission 日志 + TelePost durable 行）。** 在那之前，本页**不得**被读作
> 「已在生产验证」：两个时钟都已配置上线，但双时钟在真实 occurrence 上的收敛仍是**待验证**状态。
