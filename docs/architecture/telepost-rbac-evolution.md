...
Status: IMPLEMENTED + PRODUCTION VERIFIED (TelePost 2.47.0；role_bindings 表已在 bot1/bot2 生产库迁移；Admin Control Plane 面板已上线)
Scope: TelePost
...
- Role Binding 模型：VERIFIED（TelePost v2.46.0 已 release + 上线；bot1/bot2 生产 DB 均存在 role_bindings；Admin Control Plane 面板于 v2.47.0 上线，可在小程序「管理」页绑定/移除 reviewer/admin 角色，真实 Telegram 用户授权端到端已可用。）
