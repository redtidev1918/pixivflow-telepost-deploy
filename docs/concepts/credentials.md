...
`second-clock` 列为 `invalid`，禁止的是第二个 **PRIMARY** 时钟／第二个执行权威，因为
「重复触发是幂等的，凭据争用不是」。本套件**没有**跨平面的分布式锁、租约或围栏令牌，这是刻意的
终局决定——不变量是「只有一个执行权威」，而不是「多个权威用锁协调」。
生产因此用两个**共享同一个 durable 执行权威**的外部时钟（矩阵 `redundant-external-clock`）：
它们不需要锁，因为重复触发本来就收敛到同一个 slot。理由见 [roles.md](./roles.md)。
