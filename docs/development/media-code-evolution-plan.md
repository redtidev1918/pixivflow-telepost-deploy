...
| 状态 | 说明 |
|---|---|
| VERIFIED（Release） | TelePost 2.52.0：publish_from_file_ids 已接入 DeliveryPlanner 并调用 `mark_delivered_for_chain`；2.53.0 再叠加 reaction-based heat。运行时见 fly/deploy.telepost.toml 的 2.53.0 pin 与 /health version=2.53.0 |
| VERIFIED（DB） | media_asset_refs.file_id/file_unique_id 列在 bot1/bot2 生产库 VERIFIED；message_reaction_counts 表随 2.53.0 部署自动创建 |
